...
if True:
    clip_model, preprocess = clip.load("ViT-L/14", device=device, download_root="/fred/oz339/zdeng/.cache")
else:
    clip_model = None
    preprocess = None
...
class BLEURewardFunction:
    def __call__(self, completions, real, **kwargs):
        """
        计算 BLEU 奖励，根据分段函数：
        当 BLEU(x, x̂) > 0.5 时，R = R* × BLEU(x, x̂)
        当 BLEU(x, x̂) ≤ 0.5 时，R = -R* × (1 - BLEU(x, x̂))
        其中 R* = 1
        :param completions: list of str, 模型生成的句子
        :param real: list of str, 真实目标句子
        ...
        for c, r in zip(completions, real):
            # 计算原始 BLEU 分数
            bleu_score = sentence_bleu([r.split()], c.split(), weights=weights, smoothing_function=smoothing)
            
            # 根据分段函数计算奖励
            if bleu_score > 0.5:
                reward = R_star * bleu_score
            else:
                reward = -R_star * (1 - bleu_score)

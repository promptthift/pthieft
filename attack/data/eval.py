...
def get_model_processor(name, device=device):
    if name == "CLIP":
        ...
        model.load_state_dict(torch.load('./.cache/CSD/csd_clip_model_pytorch.pt', 
                                         map_location=device, 
                                         weights_only=True))
        model.eval()
        model = model.to(device)
...
def main():
    ...
    json_path = './sample_output/infer_sample.json'
    ...
    for idx, row in tqdm(enumerate(data), total=len(data)):
        try:
            generated_caption = row['inferred_prompt']
    ...
    output_path = './new_evaluation_results.json'

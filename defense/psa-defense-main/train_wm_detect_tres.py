...
def train_and_test(model, transform_train, transform_test, aug_test, args, device, fc_dict=None):
    ...
    # params
    data_aug = data_augmentation.HiddenAug(448, args.p_crop, args.p_blur, args.p_jpeg, args.p_rot,
    ...
    orig_image_paths = [
        f"output/lexica_wm_target/{str(sample_id).zfill(4)}_origin_{str(i).zfill(2)}.jpg" \
            for i in range(bs)
    ]
    edit_image_paths = [
        f"output/lexica_wm_target/{str(sample_id).zfill(4)}_target_{str(i).zfill(2)}.jpg" \
            for i in range(bs)
    ]
    ...
    fc = nn.Sequential(
        ...
        nn.Flatten(),  # 将 [batch_size, 2048, 1, 1] 转为 [batch_size, 2048]
        nn.Linear(2048, args.num_bits)
    ).to(device)
    ...
    if fc_dict is not None:
        ...
        for _ in range(iters):
            ...
            with torch.no_grad():
                target_features = model.body(imgs_train)
                target_features = model.global_pool(target_features)
    ...
    for name, aug in aug_test.items():
        ...
        with torch.no_grad():
            target_features = model.body(resize(aug(images)))
            target_features = model.global_pool(target_features)
            fts = fc(target_features)
    ...
    test_path_target = [
        # f"output/lexica_wm_target/{str(sample_id).zfill(4)}_target_{str(i).zfill(2)}.jpg" \
        #     for i in range(bs, 2*bs)
        f"output/lexica_psa_{tm}/{str(sample_id).zfill(4)}_{str(i).zfill(2)}.jpg" \
            for i in range(bs)
    ]
    images = [Image.open(image_path).convert("RGB") \
...
def main(args):
    ...
    args_mld = ml_decoder_args()
    args_mld.load_pretrain = True
    model = create_model(args_mld).to(device)
    ...
    tm = 'wb' if args.wb else 'bb'
    os.makedirs(os.path.join('output', f'fc_{tm}'), exist_ok=True)
    for i in range(num_samples):
        save_dict = None
        load_path = os.path.join('output', f'fc_{tm}', f'fc_{str(i).zfill(4)}.pth')

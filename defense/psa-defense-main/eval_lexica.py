...
def evaluate_prompt_stealer(prompt_stealer: PromptStealer, val_dataset, save_path="output/PS_results/"):
    prompt_stealer.eval()
    preds = []
    ...
    for i in range(prompt_stealer.args.num_samples):
        image, prompt, target, indice = val_dataset[i]
        image = Image.open(f'data/lexica_sd_v1-4/{str(i).zfill(4)}.png')
        image = transform_val(image).unsqueeze(0)
        with torch.no_grad():
            inferred_prompts, pred_modifiers = prompt_stealer.infer(image, lexica_dataset=val_dataset)
        target_modifiers = val_dataset.getCategoryListByArray(target.cpu().numpy())

        preds.append({"id": i, "prompt": prompt, "inferred_prompt": inferred_prompts[0], "target_modifiers": target_modifiers[0], "pred_modifiers": pred_modifiers[0]})
...
def main():
    ...
    evaluate_prompt_stealer(prompt_stealer, val_dataset, save_path)
